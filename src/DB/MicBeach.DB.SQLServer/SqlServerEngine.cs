    ...
    public class SqlServerEngine:IDbEngine
    {
            ...
            else
            {
                switch (cmd.Operate)
                {
                    ...
                    case OperateType.Update:
                        ...
                        cmdText.AppendFormat("UPDATE {0} SET {1} FROM [{2}] AS {0} {3}", queryTranslator.ObjectPetName, string.Join(",", updateSetArray.ToArray()), cmd.ObjectName, conditionString);
                        break;
        ...
        public IEnumerable<T> Query<T>(ServerInfo server, ICommand cmd)
        {
            ...
            switch (cmd.Query.QueryType)
            {
                ...
                default:
                    ...
                    cmdText.AppendFormat("SELECT {0} {3}.{1} FROM [{2}] AS {3}", size > 0 ? "TOP " + size : "", string.Join(fieldSplitChar, FormatFields(cmd.Fields)), cmd.ObjectName, queryTranslator.ObjectPetName);
                    if (!tranResult.ConditionString.IsNullOrEmpty())
                    {
        ...
        public IEnumerable<T> QueryOffset<T>(ServerInfo server, ICommand cmd, int offsetNum = 0, int size = int.MaxValue)
        {
            ...
            switch (cmd.Query.QueryType)
            {
                ...
                default:
                    string fieldSplitChar = string.Format(",{0}.", queryTranslator.ObjectPetName);
                    cmdText.AppendFormat("SELECT COUNT({3}.[{0}]) OVER() AS PagingTotalCount,{3}.{1} FROM [{2}] AS {3}", cmd.Fields.ElementAt(0), string.Join(fieldSplitChar, FormatFields(cmd.Fields)), cmd.ObjectName, queryTranslator.ObjectPetName);
                    if (!tranResult.ConditionString.IsNullOrEmpty())
                    {
                        cmdText.AppendFormat(" WHERE {0}", tranResult.ConditionString);
                    }
