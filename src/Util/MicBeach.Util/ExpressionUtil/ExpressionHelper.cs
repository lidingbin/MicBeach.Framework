...
namespace MicBeach.Util.ExpressionUtil
{
    ...
    {
        static Dictionary<string, dynamic> propertyOrFieldFunctions = new Dictionary<string, dynamic>();//Property or Field access Methods
        static object _lockFunctionObj = new object();
        ...
        public static Func<T, dynamic> GetPropertyOrFieldFunction<T>(string name)
        {
            ...
            Type type = typeof(T);
            string funKey = type.FullName + "_" + name;
            if (propertyOrFieldFunctions.ContainsKey(funKey))
            {
                return propertyOrFieldFunctions[funKey];
            }
            lock (_lockFunctionObj)
            {
                if (propertyOrFieldFunctions.ContainsKey(funKey))
                {
                    return propertyOrFieldFunctions[funKey];
                ...
                propertyOrFieldFunctions.Add(funKey, function);
                return function;
            }
        }
