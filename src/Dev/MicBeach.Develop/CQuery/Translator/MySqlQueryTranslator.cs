    ...
    public class MySqlQueryTranslator : IQueryTranslator
    {
        ...
        string TranslateCriteria(Criteria criteria, Dictionary<string, object> parameters, string objectName)
        {
            ...
            if (valueQuery != null)
            {
                ...
                return string.Format("{0}.`{1}` {2} (SELECT {3}.`{4}` FROM `{5}` {6} {7} {8} {9})", objectName, criteria.Name, sqlOperator, subObjName, valueQuery.QueryFields[0], valueQuery.ObjectName, subObjName, conditionString, subQueryResult.OrderString, topString);
            }
